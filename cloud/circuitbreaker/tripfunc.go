...
func ThresholdTripFunc(threshold int64) TripFunc{
	return func(m Metricer) bool { return m.Failures()+m.Timeouts() >= threshold}
}
...
func ConsecutiveTripFuncV2(rate float64, minSamples int64, duration time.Duration, durationSamples, conseErrors int64)TripFunc{
	return func(m Metricer) bool {
		samples := m.Samples()
		// based on stat
		if samples >= minSamples && m.ErrorRate() >= rate { return true}
		// based on continuous time
		if duration > 0 && m.ConseErrors() >= durationSamples && m.ConseTime() >= duration { return true}
		// base on consecutive errors
		if conseErrors > 0 && m.ConseErrors() >= conseErrors { return true}
		return false
	}
}
