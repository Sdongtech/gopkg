...
func NewPanel(changeHandler PanelStateChangeHandler,
	...
	ti, _ := tickerMap.LoadOrStore(p.defaultOptions.BucketTime,
		&shardTicker{panels: make(map[*panel]struct{}), stopChan: make(chan bool, 1)})
	t := ti.(*shardTicker)
	t.Lock()
	t.panels[p] = struct{}{}
	if !t.started {
...
// IsAllowed .
func (p *panel) IsAllowed(key string) bool {return p.getBreaker(key).IsAllowed()}

// GetMetricer ...
func (p *panel) GetMetricer(key string) Metricer{return p.getBreaker(key)Metricer()}
...
func (t *shardTicker) tick(ticker *time.Ticker){
	defer ticker.Stop()
	for {
		select {
		case <-ticker.C:
			...
			for p := range t.panels{
				p.breakers.Range(func(_ string, value interface{}) bool{
					if b, ok:= value.(*breaker); ok {b.metricer.tick()}
					return true
				})
			}
			t.Unlock()
