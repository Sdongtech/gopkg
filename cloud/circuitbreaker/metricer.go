...
func (b *bucket) Failures() int64 { return atomic.LoadInt64(&b.failure) }

func (b *bucket) Successes() int64 { return atomic.LoadInt64(&b.success) }

func (b *bucket) Timeouts() int64 { return atomic.LoadInt64(&b.timeout) }
...
func newWindowWithOptions(bucketTime time.Duration, bucketNums int32) (metricer, error){
	if bucketNums < 100 { return nil, fmt.Errorf("BucketNums can't be less than 100")}
	w := new(window)
	w.rw = syncx.NewRMMutex()
...
func (w *window) Timeout() {
	...
	b := w.getBucket()
	atomic.AddInt64(&w.conseErr, 1)
	atomic.AddInt64(&w.allTimeoutm, 1)
	if atomic.LoadInt64(&w.errStart) == 0{
		atomic.StoreInt64(&w.errStart, time.Now().UnixNano())
	}
	w.rw.Unlock()
...
func (w *window) ErrorRate() float64{
	...
	if (successes+failures+timeouts)==0 {return 0.0}
...
func (w *window) getBucket() *bucket { return &w.buckets[atomic.LoadInt32(&w.latest)]}
